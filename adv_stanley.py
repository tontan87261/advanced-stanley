...
with open("/home/tonis/Downloads/tera_path.csv", "r") as csvfile:
...
def get_target_location(x, y):
        i = 0
        min_dist = 100000000
        for i in range(len(x)):
            current_min_dist = math.dist([x[i], y[i]], [x, y])
...
def robot_speed(x,y):                                                   #input current robot coordinates
    curr_time = time.time()
    delta_t = curr_time - prev_time                                     #calculate time delta between current and previous loops 
    distance = np.sqrt(x - prev_loc_x) + np.square(y - prev_loc_y)      #distance between this and last point
    direction = np.arctan2(y - prev_loc_y, x - prev_loc_x)
    velocity = distance / delta_t                                       #robot speed (m/s)
    prev_loc_x = x                                                      #update previous x and y coords
    prev_loc_y = y                                                      #update previous x and y coords
...
def kappa_feedforward(x, y, v, psi, t_ff, kappa):                       #Inputs: x and y coordinates of robot, robot speed, robot directon, feedforward time
    ...
    ff_target = get_target_location(x_ff, y_ff)
    return kappa[ff_target[2]]

def advanced_stanley():
    #Inputs:
    gps = GPS_Subscriber()                                              # init gps subscriber class
    x_gps, y_gps = gps.get_ENU()                                        #Reference x and y coord (Position of gps)                            
    v_r, delta_time, psi_r = robot_speed()                                     #v_r = robot speed (m/s), time since last iteration (Seconds), direction of robot
    kappa = calculate_curvature()                                       #Curves of each path point (array)
    nearest_point_r = get_target_location(x_gapped, y_gapped)
    kappa_ref = kappa[nearest_point_r[2]]                               #curvature of path at the nearest path point
    path_dirs, path_lens= path_vectors()
    psi_r_ref = path_dirs[nearest_point_r[2]]                           #direction of the path at the nearest path point
    x_r_ref = nearest_point_r[0]                                        #x coord of closest point of path to rear axle
    y_r_ref = nearest_point_r[1]                                        #y coord of closest point of path to rear axle
    
    delta_yaw = delta - delta_prev                                      # compute yaw difference between samples
    ...
    kappa_ref_ff = kappa_feedforward(x_gps, y_gps, v_r, psi_r, t_ff, kappa)
    ...
    delta_add = k_d_yaw * (psi_dot_kappa_ref - psi_dot_r) * sign_v_r + k_d_steer * (delta_prev - delta) + theta_ss_f        
    ...
    delta = delta_stanley_enhanced          #Update current steering value
    if delta_prev != delta:                 #Update previous steering value (omalooming)
        delta_prev = delta
...
if __name__ == '__main__':
    ...
    while True:
        rclpy.spin_once(gps)
        #Calculate steering
        steering = advanced_stanley()
        steering_clipped = np.clip(np.rad2deg(steering), -max_steer, max_steer)
        print(f"Steeing value in rads: {np.rad2deg(steering)}")
